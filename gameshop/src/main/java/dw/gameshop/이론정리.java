...
팁
 ...
 - 욕심은 금물! 기본 기능 먼저 구현
 - 강사님과 상의를 하자
 - 레퍼런스 자료를 많이 확보할수록 설계가 용이함



[ Refactoring ]
 // 이론
리펙토링 : 항상 개발을 마무리 하고 코드를 가시성 있게 정리하는 과정이 필요하다. 코드의 배치를 바꾸고, 중복을 지우는 등 다시 빌드를 한다는 의미이다.
리펙토링의 대상 :
1. 중복 메서드 선언 정리
2. 변수 이름 메서드
3. 메서드 호출 구조 일원화
...
[ HTML ]
 // 이론
HTML : Hyper Text Markup Language : 내가 원하는 페이지로 이동하도록 설정되어있는 정보 언어. (문서)
구조 이론:
    HTML
          |
          \-HEAD
...
요소 :
 - 여는태그와 닫는 태그 사이에 있는 것을 '요소'라고 부르며, 요소는 ELEMANT라고 부른다.
이 외 나머지 사용법은 vcs 에서 문서-HTML&CSS 파일 참조
 // 용어
ietf : 인터넷 국제 표준 기구
...
[ JavaScript ]
 // 이론
JS : JavaScript
기원 : 과거에는 정적 페이지 밖에 없었다. 그래서 HTML 과 CSS 만으로 웹 페이지 프론트앤드 부분을 만들었다.
      그런데 기술이 발전하며 사람들은 정적 페이지 만으로 만족하지 못하기 시작했고,
      동적 페이지를 원했다(ex : 뉴스의 헤드라인이 계속 바뀌는 기능, 유튜브 새로운 영상 추천등).
      그래서 기존에 쓰던 HTML 과 CSS 로는 구현할 수 없어
      Netscape 라는 회사에서 빠른 로딩이 가능한 인터프릿 언어인 JavaScript 를 발명하게 된다.
      CSR 이 유행을 하면서 접근성이 좋아 인지도는 수직상승하고 IE의 인정을 받고, 경쟁자 없이 동적 페이지의 유일한 절대 강자로 떠오른다.
      또한 브라우저를 의존하여 사용하던 때와 달리 지금은 Node 를 이용해 서버단에서도 JS를 컴파일 할 수 있는, 웹 뿐아닌 모든 분야의 만능이 되었다.
이론 : JavaScript 는 스크립트, 인터프릿 언어이다. 즉 실시간 로딩이 가능하다.
      CSR 에서 주로 쓰이며, 빠른 로딩의 장점으로 동적페이지를 구현하는데 사용한다.
      Node 나 브라우저를 이용해 컴파일을 한다.
JavaScript 사용분야 :
 1. 웹의 요소를 제어한다.
    - 태그 사이의 요소를 동적으로 만들어준다.
 2. 웹 애플리케이션 재작
 3. 서버 개발
    - Node 를 사용하여 개발
 // 사용법
변수 선언 키워드 :
 - JavaScript 는 변수 타입을 선언하지 않는다. 과거엔 Var 하나로 변수 선언했으나 지금은 let 과 const 로 나뉘었다.
   인터프릿 언어라 타입을 선언할 필요가 없어서 타입 선언을 하지않기 때문에 메모리 공간 효율이 떨어지는 면이 있다.
 1. Var : 과거엔 모든 타입에 사용했으나 지금은 '공식적'으로 사용하지 않음
 2. let : 값 수정이 가능
 3. const : 상수(값 수정이 불가능)
그래서 요즘은 Type Script 같이 자바스크립트의 기능에 변수의 타입 선언이 추가된 언어도 나오고 있다.
이 외 나머지 사용법은 vcs 에서 문서-Javascript 파일 참조
 // 용어
컴퓨터 언어의 종류 :
 1. 컴파일 언어
    - 먼저 코드를 짜고 나중에 한번에 컴파일
     장점 : 한번에 구조 논리를 파악하기 때문에 프로그램 속도가 빠르고 에러를 방지함.
     단점 : 실시간 응대 불가, 로딩 속도 느림.
 2. 인터프릿 언어
    - 코드를 한 줄씩 실행(Node, 브라우저를 컴파일러로 사용이 가능)
     장점 : 로딩속도가 빠르고 실시간 응대가 가능함.
     단점 : 에러는 실행을 시켜봐야 알 수 있음. 프로그램 속도가 느림.
Node :
 - JS의 컴파일러. JS를 브라우저뿐 아닌 서버에서도 사용이 가능하게 만드는 기능이다.
 - 대표적 프레임워크에는 NEXT 가 있다.
